...
def run_qrew():
    ...
    # --- Project Setup ---
    project_name = "FitnessTrackerApp" # Example project name
    print(f"\nStarting/Resuming project: {project_name}")

    # State manager is now primarily managed by the orchestrator,
    # but we can instantiate one here to print resume status or get project path for final output.
    # The orchestrator will create its own instance.
    # For checking resume point before orchestrator, this is fine.
    ...
    if temp_state_manager_for_info.state.get("status") == "completed":
        ...
        # This condition means it's likely a new project or ready for finalization
        # if all stages were complete but project not finalized.
        # The orchestrator handles this logic.
    ...
    pipeline_inputs = {
        "project_name": project_name, # Important for state management within workflows
    ...
    print(f"\nInitiating data pipeline with inputs for project '{project_name}'...")
    ...
    try:
        ...
        if final_state_manager.state["status"] == "completed":
            print("Project successfully completed and finalized.")
            final_output_artifact = results.get("final_assembly", {}) # Get the final assembly output
            if not final_output_artifact and results:
                 # If final_assembly is empty, grab all artifacts as final output
                final_output_artifact = results

            print("Final Output Artifacts:", json.dumps(final_output_artifact, indent=2))

            # Save the final aggregated output from the pipeline to a file in the project directory
            # The 'results' variable contains all artifacts from all stages.
            # We can save this comprehensive result.
        ...
        elif final_state_manager.state["status"] == "failed":
            print(f"Project execution failed at stage: {final_state_manager.state.get('current_stage')}")
            print("Review the Workflow Summary printed above for error details.")
        else:
            print(f"Project execution finished with status: {final_state_manager.state['status']}. Not all stages may have completed.")
