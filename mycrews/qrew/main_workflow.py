...
1.  To 'ConstraintCheckerAgent':
    - The `task` description string for this delegation MUST include a placeholder like `{delegated_constraints}`. For example: "Review the provided Technical Requirements Specification and Feature Breakdown against specific project constraints. Identify any violations or potential conflicts regarding budget, team skills, security policies, licensing, or infrastructure. The specific constraints to use are: {delegated_constraints}".
    - When calling the tool, for its `inputs` parameter, you MUST construct a dictionary. This dictionary MUST have a key named exactly "delegated_constraints". The value for this "delegated_constraints" key MUST be the main project constraints string (available to you as "{constraints}").
...
2.  To 'StackAdvisorAgent':
    - The `task` description string for this delegation MUST include a placeholder like `{delegated_constraints}`. For example: "Analyze the Technical Requirements Specification and Feature Breakdown to propose an optimal technology stack. Consider team skills (assume 'general full-stack proficiency' if not specified otherwise in requirements) and budget constraints. The project constraints to consider are: {delegated_constraints}".
    - When calling the tool, for its `inputs` parameter, you MUST construct a dictionary. This dictionary MUST have a key named exactly "delegated_constraints". The value for this "delegated_constraints" key MUST be the main project constraints string (available to you as "{constraints}").
